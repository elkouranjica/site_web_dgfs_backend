...
class Etablissement(AbstractModel):
    ...
    categorie = models.CharField(choices=[('ES', 'Etablissement Spécilaisé'),
                                          ...
                                          ('CHRR', 'Centre Hospitalier de Référence de '
                                                   'Région'),
                                          ('CHRD', 'Centre Hospitalier de Référence de '
                                                   'District'),
                                          ('CSB1', 'Centre de Santé de Base 1'),
                                          ('CSB2', 'Centre de Santé de Base 2')],
                                 max_length=4)
    adresse = models.CharField(max_length=200)

    logo = models.ImageField(upload_to=logo_directory_path, null=True, blank=True)
    image_description = models.ImageField(upload_to=description_directory_path, null=True,
                                          blank=True)
...
class Personnel(AbstractModel):
    etablissement = models.ForeignKey(Etablissement, related_name='personnels',
...
class Description(AbstractModel):
    etablissement = models.ForeignKey(Etablissement, related_name='descriptions',
                                      on_delete=models.CASCADE)
...
class Contact(AbstractModel):
    etablissement = models.ForeignKey(Etablissement, related_name='contacts',
                                      on_delete=models.CASCADE)
    ...
    contact_ambulance = models.CharField(max_length=10, unique=True, null=True,
                                         blank=True)
    ...
    def acceuil(self):
        return " ".join([self.contact_accueil[:3], self.contact_accueil[3:5],
                         self.contact_accueil[5:8], self.contact_accueil[8:]]) if \
            self.contact_accueil else ""

    @admin.display(description="Urgence")
    def urgence(self):
        return " ".join([self.contact_urgence[:3], self.contact_urgence[3:5],
                         self.contact_urgence[5:8], self.contact_urgence[8:]]) if \
            self.contact_urgence else ""

    @admin.display(description="Ambulance")
    def ambulance(self):
        return " ".join([self.contact_ambulance[:3], self.contact_ambulance[3:5],
                         self.contact_ambulance[5:8], self.contact_ambulance[8:]]) if \
            self.contact_ambulance else ""
    ...
    def daf(self):
        return " ".join([self.contact_daf[:3], self.contact_daf[3:5],
                         self.contact_daf[5:8], self.contact_daf[8:]]) if \
            self.contact_daf else ""

    @admin.display(description="DAT")
    def dat(self):
        return " ".join([self.contact_dat[:3], self.contact_dat[3:5],
                         self.contact_dat[5:8], self.contact_dat[8:]]) if \
            self.contact_dat else ""


# Service : ex = ORL, Maternité, Pédiatrie, ...
class Service(AbstractModel):
    etablissement = models.ForeignKey(Etablissement, related_name='services',
                                      on_delete=models.CASCADE)
